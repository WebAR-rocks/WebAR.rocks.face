...
const WebARRocksFaceFlexibleMaskHelper = (function () {
  ...
  _settings.debugKeypointInfluencesRendering =
    _settings.debugKeypointInfluencesRendering ||
    _settings.debugKeypointInfluencesCount;
  ...
  function add_edge(ia, ib, edgeGraph, edges) {
    ...
    if (
      edgeGraph[ifrom].some(function (edge) {
        return edge.to === ito;
      })
    ) {
      return;
  ...
  function test_ifQuad(A, B, C, D) {
    ...
    const maxDot = Math.max(
      Math.abs(ABu.dot(BCu)),
      Math.abs(BCu.dot(CDu)),
      Math.abs(CDu.dot(DAu)),
      Math.abs(DAu.dot(ABu))
    );
    return maxDot < 0.2;
  }
  // END MISC HELPERS
  ...
  const that = {
    load_geometryFromGLTF: function (
      loadingManager,
      GLTFUrl,
      flexibleGeometryName
    ) {
      return new Promise(function (resolve, reject) {
        new THREE.GLTFLoader(loadingManager).load(GLTFUrl, function (model) {
          const geom = that.extract_flexibleMaskGeometry(
            model.scene,
            flexibleGeometryName
          );
          resolve(geom);
    ...
    extract_flexibleMaskGeometry: function (scene, geomName) {
      let threeGeom = null,
        threeGeomParent = null,
        err = null;
      scene.traverse(function (threeStuff) {
        if (
          (threeStuff.type !== "Mesh" && threeStuff.type !== "SkinnedMesh") ||
          !threeStuff.geometry
        ) {
          return;
        }
        const isNameValid =
          !geomName || (geomName && geomName === threeStuff.name);
        if (!isNameValid) {
          return;
        }
        if (threeGeom === null) {
      ...
      for (let i = 0; i < geom.attributes.position.count; ++i) {
        vertices.push(
          new THREE.Vector3().fromArray(
            geom.attributes.position.array.slice(3 * i, 3 * i + 3)
          )
        );
      }
    ...
    add_diagonalEdges: function (geom) {
      ...
      console.log(
        "INFO in WebARRocksFaceFlexibleMaskHelper: ",
        counterDiagonalEdgesParams.length,
        "counter diagonal edges have been found"
      );
    ...
    compute_closestKeypointsEdgeDistances: function (geom, keypoints) {
      ...
      console.log(
        "Info in flexible mask helper: there are ",
        vertices.length,
        "vertices in the flexible geometry"
      );
      ...
      // init browsing:
      let borderPointIndicesByKp = keypoints.map(function (
        keypoint,
        keypointIndice
      ) {
      ...
      do {
        ...
        const newBorderPointIndicesByKp = borderPointIndicesByKp.map(function (
          borderPointIndices,
          kpIndice
        ) {
          ...
          borderPointIndices.forEach(function (borderPointIndice) {
            const connectedEdges = get_connectedEdges(
              edgeGraph,
              verticesEqs[borderPointIndice],
              borderPointIndice
            );
            const edgesNotBrowsed = connectedEdges.filter(
              is_edgeNotBrowsedForKp.bind(null, kpIndice)
            );
            edgesNotBrowsed.forEach(function (edge) {
              // set edge as browsed for this kp:
              edge.browsedFor.push(kpIndice);

              const neighborIndice = get_edgeOtherPointIndice(
                edge,
                borderPointIndice
              );
              ...
              if (
                keypointsInfluences[neighborIndice].some(function (
                  kpInfluence
                ) {
              ...
              verticesEqs[neighborIndice].forEach(function (
                neighborIndiceOrEq
              ) {
    ...
    compute_keypointsInfluences: function (keypoints, keypointsInfluences) {
      ...
      const edgeDistanceMeansPerKeypointCount = new Uint16Array(
        keypoints.length
      );
      ...
      for (let i = 0; i < keypoints.length; ++i) {
        if (edgeDistanceMeansPerKeypointCount[i]) {
          edgeDistanceMeansPerKeypoint[i] /=
            edgeDistanceMeansPerKeypointCount[i];
        }
      }
      ...
      keypointsInfluences.forEach(function (influences, i) {
        ...
        if (influences.length === 2) {
          ...
          const sumEdgeDistances =
            influences[1].edgeDistance + influences[0].edgeDistance;
          keypointsMorphInfluences[3 * i] =
            influences[1].edgeDistance / sumEdgeDistances;
          keypointsMorphInfluences[3 * i + 1] =
            influences[0].edgeDistance / sumEdgeDistances;
          return;
        }
    ...
    decay_keypointsMorphInfluences: function (
      geom,
      keypoints,
      keypointsMorphInfluences,
      decayRange
    ) {
      const decayStart = decayRange[0],
      ...
      for (let i = 0; i < n; ++i) {
        ...
        position.fromArray(
          positions.slice(3 * pointIndice, 3 * pointIndice + 3)
        );
        const d = position.distanceTo(keypointPosition);

        // compare this distance to decay metrics:
        if (d < decayStart) {
          // no decay, too close to the keypoint
          continue;
        } else if (d > decayEnd) {
          // cancel keypoint influence because too far from the keypoint:
    ...
    debug_keypointInfluencesCoeffs: function (
      keypoints,
      keypointsInfluences,
      keypointsMorphInfluences,
      indice
      ...
      const indices = Array.prototype.slice.call(
        keypointsMorphInfluences.indices.slice(iStart, iEnd)
      );
      const influences = Array.prototype.slice.call(
        keypointsMorphInfluences.morphInfluences.slice(iStart, iEnd)
      );
      ...
      console.log(
        "INFO in debug_keypointInfluencesCoeffs(): debug point with indice",
        indice
      );
    ...
    bind_keypointsMorphInfluenceToGeom: function (
      geom,
      keypointsMorphInfluences
    ) {
      // build bufferAttributes:
      const kpIndicesBA = new THREE.BufferAttribute(
        keypointsMorphInfluences.indices,
        3,
        false
      );
      const kpMorphInfluenceBA = new THREE.BufferAttribute(
        keypointsMorphInfluences.morphInfluences,
        3,
        false
      );
    ...
    build_debugKeypointsMorphMesh: function (geom, keypointsCount) {
      ...
      const vertexShader =
        ...
        uniform vec3 kpColors[" +
        keypointsCount.toString() +
        "];\n\
        varying vec3 vCol;\n\
        void main() {\n\
    ...
    build_flexibleMaskMaterial: function (originalMaterial, keypointsCount) {
      ...
      mat.onBeforeCompile = function (shaders) {
        ...
        // tweak vertex shader:
        const GLSLDeclareVars =
          "attribute vec3 kpIndices, kpMorphInfluences;\n\
          uniform vec3 kpDisplacements[" +
          keypointsCount.toString() +
          "];\n";
        shaders.vertexShader = tweak_threeShaderAdd(
          shaders.vertexShader,
          "#include <common>",
          GLSLDeclareVars
        );
        const GLSLDisplace =
          "vec3 displaced = vec3(0.0, 0.0, 0.0);\n\
    ...
    build_flexibleMaskFromStdMetadata(
      scene,
      ARTrackingFaceMetadata,
      updateSolvePnP
    ) {
      const geom = that.extract_flexibleMaskGeometry(
        scene,
        ARTrackingFaceMetadata["DEFORMEDID"]
      );
      ...
      const flexibleMaskMesh = that.build_flexibleMask(
        geom,
        flexibleMaskPoints,
        {
          kpInfluenceDecay:
            ARTrackingFaceMetadata["DEFORMINFLUCENCERANGE"] ||
            _defaultBuildOptions.kpInfluenceDecay,
        }
      );
    ...
    build_flexibleMask: function (geom, face3DKeypointsPositions, optionsArg) {
      ...
      const keypointsInfluences = that.compute_closestKeypointsEdgeDistances(
        geom,
        keypoints
      );
      const keypointsMorphInfluences = that.compute_keypointsInfluences(
        keypoints,
        keypointsInfluences
      );
      if (_settings.debugKeypointInfluencesCoeffs !== -1) {
        that.debug_keypointInfluencesCoeffs(
          keypoints,
          keypointsInfluences,
          keypointsMorphInfluences,
          _settings.debugKeypointInfluencesCoeffs
        );
      }
      that.decay_keypointsMorphInfluences(
        geom,
        keypoints,
        keypointsMorphInfluences,
        options.kpInfluenceDecay
      );
      ...
      const mat = that.build_flexibleMaskMaterial(
        originalMaterial,
        keypointsCount
      );
    ...
    compute_face3DKeypointsPositionsVp: function (camera, mesh) {
      ...
      keypoints.forEach(function (kp) {
        ...
        kp.positionVpProjected.set(
          kp.positionClip.x / kp.positionClip.w,
          kp.positionClip.y / kp.positionClip.w
        );
      });
    },
    ...
    compute_keypointsDisplacements: function (camera, mesh) {
      ...
      keypoints.forEach(function (kp) {
        ...
        kp.displacementVp
          .copy(kp.positionVpMeasured)
          .sub(kp.positionVpProjected);
        ...
        kp.displacementView.setX(
          (-zClip * kp.displacementVp.x) / matProj.elements[0]
        );
        kp.displacementView.setY(
          (-zClip * kp.displacementVp.y) / matProj.elements[5]
        );
    ...
    update_flexibleMask: function (camera, mesh, detectStates, landmarks) {
      if (detectStates.length) {
        throw new Error(
          "Flexible mask currently only works for single face detection"
        );
      }
