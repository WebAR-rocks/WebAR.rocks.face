...
class VTOGlasses extends Component {
  ...
  componentDidMount(){
    ...
    try {
      ...
      /* handle the error */
      console.error("ERROR WITH GET USER MEDIA");
    }
      ...
      .then((devices) => {
        devices.forEach( (device) => {
          ...
          if (device.kind === 'videoinput') {
            ...
            // WARNING: If you have multiple cameras, this may break (it works for now because I have just one camera)
  ...
  render(){
    ...
    return (
      ...
      {/* Canvas managed by WebAR.rocks, just displaying the video (and used for WebGL computations) */}
        ...
        <video className='mirrorX' ref='video' style={{position: 'fixed', zIndex: 1, objectFit: "cover", ...this.state.sizing}} autoPlay/>
